...
export default async function DashboardPage() {
  ...
  const [{ data: players }, { data: activeRoster }, { data: unpaidSignups }] = await Promise.all([
    supabase
      .from("players")
    ...
    supabase
      .from("roster")
      .select("player_id, jersey_number, teams(id, name, sport, season, age_group, organization, mojo_code, snack_signup_enabled)")
  ...
  const activeTeamIds = [...teamMap.keys()];

  // Next upcoming game per active team + parent's snack signups
  ...
  return (
    <div>
      ...
      {teamEntries.length === 0 && (
        <p className="text-sm text-gray-500 dark:text-gray-400">No active seasons right now.</p>
      )}

      <div className="space-y-4">
        {teamEntries.map(({ team, playerIds: pids }) => {
          const nextGame = nextGameByTeam.get(team.id);
          const isSignedUp = nextGame ? signedUpGameIds.has(nextGame.id) : false;

          return (
            <div key={team.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
              {/* Team header */}
              <Link
